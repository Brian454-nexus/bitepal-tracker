...
public abstract class Random {
    ...
    public static /* synthetic */ byte[] nextBytes$default(Random random, byte[] bArr, int i6, int i7, int i8, Object obj) {
        ...
        return random.nextBytes(bArr, i6, i7);
    }
    ...
    public byte[] nextBytes(@NotNull byte[] array, int fromIndex, int toIndex) {
        ...
        if (fromIndex < 0 || fromIndex > array.length || toIndex < 0 || toIndex > array.length) {
            throw new IllegalArgumentException(("fromIndex (" + fromIndex + ") or toIndex (" + toIndex + ") are out of range: 0.." + array.length + b.f11260a).toString());
        }
        if (fromIndex <= toIndex) {
            ...
            for (int i9 = 0; i9 < i8; i9++) {
                array[fromIndex + i9] = (byte) (nextBits >>> (i9 * 8));
            }
            return array;
        }
    ...
    public double nextDouble(double from, double until) {
        ...
        double d6 = until - from;
        if (Double.isInfinite(d6) && !Double.isInfinite(from) && !Double.isNaN(from) && !Double.isInfinite(until) && !Double.isNaN(until)) {
            double d7 = 2;
            double nextDouble2 = nextDouble() * ((until / d7) - (from / d7));
            nextDouble = from + nextDouble2 + nextDouble2;
        } else {
            nextDouble = from + (nextDouble() * d6);
        }
    ...
    public long nextLong(long from, long until) {
        ...
        if (j8 > 0) {
            if (((-j8) & j8) == j8) {
                ...
                int i7 = (int) (j8 >>> 32);
                if (i6 != 0) {
                    nextInt = nextBits(RandomKt.fastLog2(i6));
                } else if (i7 == 1) {
                    nextInt = nextInt();
                } else {
                    j7 = (nextBits(RandomKt.fastLog2(i7)) << 32) + (nextInt() & 4294967295L);
                    return from + j7;
                }

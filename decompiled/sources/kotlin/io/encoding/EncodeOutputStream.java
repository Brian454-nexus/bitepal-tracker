...
final class EncodeOutputStream extends OutputStream {
    ...
    public void write(@NotNull byte[] source, int offset, int length) {
        ...
        checkOpen();
        if (offset < 0 || length < 0 || (i6 = offset + length) > source.length) {
        ...
        if (i7 < 3) {
            if (i7 != 0) {
                offset += copyIntoByteBuffer(source, offset, i6);
                if (this.byteBufferLength != 0) {
                    return;
                }
            }
            while (offset + 3 <= i6) {
                int min = Math.min((this.base64.getIsMimeScheme() ? this.lineLength : this.symbolBuffer.length) / 4, (i6 - offset) / 3);
                int i8 = (min * 3) + offset;
                if (encodeIntoOutput(source, offset, i8) != min * 4) {
                    throw new IllegalStateException("Check failed.");
                }
                offset = i8;
            }
            ArraysKt.copyInto(source, this.byteBuffer, 0, offset, i6);
            this.byteBufferLength = i6 - offset;
            return;
        }
        throw new IllegalStateException("Check failed.");
    }
